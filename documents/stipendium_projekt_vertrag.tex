...
\newcommand\vorsitzender{Marie Schmitz}

% Daten des Stipendiaten
\newcommand\stipendiat{NAME}		% Name
\newcommand\stipgeburt{DATUM}		% Geburtsdatum
\newcommand\stipanschrift{STADT}	% Wohnort
\newcommand\stipbetrag{GESAMTBETRAG}	% Gesamthöhe der Förderung
\newcommand\stiptagessatz{60}		% Tagessatz
\newcommand\stiptage{ANZAHL FÖRDERTAGE}	% Anzahl der geförderten Tage
\newcommand\stipstart{STARTDATUM}	% Beginn der Förderung
\newcommand\stipende{ENDDATUM}		% Ende der Förderung
\newcommand\stipkontoinhaber{}		% Kontodaten auf welches die Förderung
\newcommand\stipbic{}			% überwiesen werden soll.
\newcommand\stipiban{}			%
...
\begin{enumerate}[\qquad(1)]
	\item Der Verein PeP et al. gewährt dem/der Stipendiaten/in nach
		...
		dem/der Stipendiaten/in sein/ihr Projekt ermöglichen soll.
...
\begin{enumerate}[\qquad(1)]
	\item Der Stipendiengeber gewährt dem/der Stipendiaten/in ein
		Stipendium in Höhe von einmalig \num{\stipbetrag}\,€ für
		den Projektzeitraum vom \stipstart\ bis zum \stipende. Dies berechnet sich aus einem Tagessatz von \num{\stiptagessatz}\,€ für \num{\stiptage} Tage.
	\item Die Zahlung erfolgt einmalig im Voraus durch Überweisung auf
...
\begin{enumerate}[\qquad(1)]
	...
	\item Über den Verlauf und die Ergebnisse des Projekts legt der/die
		Stipendiat/in Zeugnis ab. Der/Die Stipendiat/in
	...
	\item Unterbricht der/die Stipendiat/in sein/ihr wissenschaftliches Vorhaben,
		so unterrichtet er/sie PeP et al. unverzüglich.
\end{enumerate}
